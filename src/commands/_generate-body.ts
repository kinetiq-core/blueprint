...
import type { ProgressCount, ProgressSnapshot } from '../lib/progress.js'
...
export async function generate(flags: Record<string, string | boolean>): Promise<void> {
...
type Section = {
  ...
  pages: Array<{ key: SectionPageKey; label: string; href: string }>
}
...
function statusTone(value: string) {
  ...
  if (v === 'ready' || v === 'closed') return 'ready'
  if (v === 'alpha' || v === 'beta' || v === 'started' || v === 'in progress' || v === 'now') return 'active'
  if (v === 'planned' || v === 'poc' || v === 'triaged' || v === 'exploratory' || v === 'later') return 'planned'
  if (v === 'parked' || v === 'blocked' || v === 'deprecated' || v === 'missing') return 'parked'
  if (v === 'open') return 'open'
  if (v === '-') return 'na'
  return ''
}

function isStatusLikeColumn(header: string) {
...
function columnClass(header: string) {
  const lc = header.toLowerCase()
  if (lc === 'source') return 'col-source'
  if (lc === 'spec') return 'col-spec'
  if (lc === 'feature') return 'col-feature'
  if (lc === 'capability') return 'col-capability'
  if (lc === 'subfeature') return 'col-subfeature'
  if (lc === 'item') return 'col-item'
  if (lc === 'notes') return 'col-notes'
  return ''
}
...
const TABLE_SCOPE: Partial<Record<TableKey, ProgressScope>> = {
...
const TABLE_GROUPING: Partial<Record<TableKey, TableGrouping>> = {
...
function sortByGrouping(rows: SnapshotRow[], grouping: TableGrouping): SnapshotRow[] {
...
function sourceSlug(sourceId: string) {
  return sourceId.replace(/^kinetiq-/, '')
}

function sourceNavLabel(label: string) {
  return label.replace(/^Kinetiq\s+/i, '')
}

function toCsv(headers: string[], rows: SnapshotRow[]) {
...
function buildSections(snapshot: Snapshot): Section[] {
  ...
  if (okSources.length <= 1) return sourceSections
...
function sectionRows(section: Section, table: SnapshotTable) {
  if (!section.source) return table.rows
...
function buildSidebar(sections: Section[], activeSectionSlug: string, activePageKey: string) {
  return `
  <aside class="sidebar">
...
const specsBySource = new Map<string, SpecMeta[]>()
for (const spec of allSpecs) {
...
for (const section of sections) {
  ...
  const sectionCards = section.pages
    ...
    .map((page) => {
      ...
      const scope = TABLE_SCOPE[page.key as TableKey]
  ...
  for (const page of section.pages) {
    ...
    writeFileSync(join(OUTPUT_DIR, csvRouteFor(section.slug, page.key as TableKey)), toCsv(table.headers, rows))
    ...
    const grouping = TABLE_GROUPING[page.key as TableKey]
    ...
    const renderedRowChunks: string[] = []
    ...
    const tableScope = TABLE_SCOPE[page.key as TableKey]
    ...
    const body = `
      <section class="hero">
        ...
        <div class="links">
          ...
          <a href="${escHtml(csvRouteFor(section.slug, page.key as TableKey))}">Download CSV</a>
        </div>
      </section>
      <section class="panel">
...
for (const spec of allSpecs) {
  ...
  for (const w of rendered.warnings) console.warn(`[specs] ${w}`)
...
if (specsRenderedCount) console.warn(`Rendered ${specsRenderedCount} spec pages${specsWarnCount ? ` (${specsWarnCount} link warnings)` : ''}`)
...
function newTreeNode(name: string): SpecTreeNode {
  return { name, children: new Map(), files: [] }
}

function insertSpec(root: SpecTreeNode, segments: string[], spec: SpecMeta): void {
...
function countTreeFiles(node: SpecTreeNode): number {
  let n = node.files.length
  for (const child of node.children.values()) n += countTreeFiles(child)
  return n
}
...
function extractPreviewRow(fm: Record<string, string>): PreviewRow {
  ...
  let type = ''
  if (hasMobileFeature && hasWebFeature) type = 'feature'
  else if (hasMobileFeature) type = 'mobile-feature'
  else if (hasWebFeature) type = 'web-feature'
  else if (fm.roadmap_type === 'feature' || fm.roadmap_feature_item) type = 'feature'
  else if (fm.roadmap_type === 'backend' || fm.roadmap_backend_item) type = 'backend'
  else if (fm.roadmap_release_item) type = 'release'
  else if (fm.roadmap_ops_item) type = 'ops'
  else if (fm.roadmap_future_item) type = 'future'

  const surfaces: string[] = []
  if (hasMobileFeature) surfaces.push('Mobile')
  if (hasWebFeature) surfaces.push('Web')

  return {
    type,
    group:
      fm.roadmap_mobile_feature_group ||
      ...
      '',
    item:
      fm.roadmap_mobile_feature_item ||
      ...
      '',
    phase:
      fm.roadmap_mobile_feature_phase ||
...
type PreviewBucket = 'shipped' | 'beta' | 'alpha' | 'planned' | 'parked'

function classifyPreviewValue(value: string): PreviewBucket | null {
...
function rollupTreeNode(node: SpecTreeNode): FolderRollup {
  const roll: FolderRollup = { shipped: 0, beta: 0, alpha: 0, planned: 0, parked: 0, tracked: 0, total: 0 }
...
function renderRollupBar(roll: FolderRollup): string {
...
function groupParentChildFiles(files: SpecMeta[]): Array<{ parent: SpecMeta; children: SpecMeta[] }> {
  const bases = new Map<string, SpecMeta>()
  for (const spec of files) {
    const base = (spec.relPath.split('/').pop() || '').replace(/\.md$/, '')
    bases.set(base, spec)
  }

  const parentBases = new Set<string>()
  ...
  const childrenByParent = new Map<string, SpecMeta[]>()
  const topLevel: SpecMeta[] = []

  for (const [base, spec] of bases) {
    if (parentBases.has(base)) {
      topLevel.push(spec)
      if (!childrenByParent.has(base)) childrenByParent.set(base, [])
      continue
    }
    let matched: string | null = null
    for (const parentBase of parentBases) {
      if (base.startsWith(parentBase + '_')) {
        if (!matched || parentBase.length > matched.length) matched = parentBase
      }
    }
    if (matched) {
      if (!childrenByParent.has(matched)) childrenByParent.set(matched, [])
...
function renderPreviewTreeNode(node: SpecTreeNode, depth: number): string {
  ...
  for (const { parent, children } of groups) {
    ...
    for (const child of children) html += renderPreviewLeaf(child, true)
  }
...
function distinctValues(specs: SpecMeta[], fieldPicker: (row: PreviewRow) => string): string[] {
...
function renderFilterSelect(field: string, label: string, values: string[]): string {
  if (!values.length) return ''
  const options = [`<option value="">All ${escHtml(label.toLowerCase())}</option>`]
  for (const v of values) options.push(`<option value="${escAttr(v)}">${escHtml(v)}</option>`)
...
function buildRootBreakdown(): string {
  ...
  const columns = sourceSections
    .map((section) => {
      ...
      const l1Blocks = l1Nodes.map((l1) => {
        ...
        const l1Header = link(
          l1.name,
          `<span class="breakdown-name">${escHtml(l1.name)}</span>
        ...
        const l2Rows = l2Nodes
          .map((l2) => {
            ...
            return link(
              `${l1.name}/${l2.name}`,
              `<span class="breakdown-name breakdown-name-sub">${escHtml(l2.name)}</span>
